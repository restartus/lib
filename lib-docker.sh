...
docker_start() {
	...
	if (($# == 0)); then
		...
		if ! sudo "$ctl" docker status | grep -q running; then
			eval sudo "$ctl" docker restart
			if ! sudo "$ctl" service docker status | grep -q running; then
				return 1
			fi
	...
	if ! docker-machine ls | grep "$host"; then
...
docker_available() {
	if ! docker ps >/dev/null 2>&1; then
		if [[ $OSTYPE =~ linux ]]; then
			if sudo service docker status | grep -q running && ! groups | grep -q docker; then
...
docker_machine_create_swarm() {
	...
	if ! docker_machine_create \
		...
		$flags; then
		if ssh "$remote" docker ps --filter "name=swarm-agent" | grep swarm-agent; then
