...
add_local() {
    ...
    do
        local ext="${host##*.}"
        if [[ ! $ext =~ local ]] && ! grep "$ext" "$TLD_LIST"
        then
            echo $host.local
        else
            echo $host
...
add_account_and_local()
{
    ...
    local account=$1
    shift
    echo $(add_local $(add_account "$account" $@))
}
...
generate_remotes()
{
    ...
    then
        ...
        do
            echo $(add_account_and_local "$account" "$host$i")
        done
    else
        shift 3
        log_verbose adding $@
        echo $(add_account_and_local "$account" $@)
    fi
}
...
remove_account_and_local() {
    for host in $@
    do
         echo $(remove_account $(remove_local "$host"))
    done
}
...
host_alive() {
    for host in $(remove_account $@)
    do
        if ! ping -c 2 "$host" 2>&1 >/dev/null
        then
            return 1
        fi
    done
