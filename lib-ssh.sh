...
ssh_install_dir() {
	...
	if [[ -d "$src" ]]; then
		...
		for secret in aws-access-key* *.pem *.id_rsa *.id_ed25519 *.json; do
			...
			if [[ -e $src/$secret && ! -e $dest/$secret || ! -L $dest/$secret ]]; then
...
use_openssh_keychain() {
	if ! dpkg -s openssh-client | grep 'Status:.* installed'; then
	...
	if (($# > 0)) && ! grep "$1" "$HOME/.ssh/config"; then
		return 1
	elif ! grep "id_ed255119" "$HOME/.ssh/config"; then
		return 1
